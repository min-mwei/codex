...
fn codex_command(codex_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::new(codex_utils_cargo_bin::cargo_bin("vorpal")?);
    cmd.env("VORPAL_HOME", codex_home);
    Ok(cmd)
}
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    assert!(stdout.contains("remove: vorpal mcp remove docs"));
