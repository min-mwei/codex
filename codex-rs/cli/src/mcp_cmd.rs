...
pub struct AddMcpStreamableHttpArgs {
    ...
    /// Optional inline bearer token to authenticate with the server.
    /// Only valid with streamable HTTP servers. Use with caution because the token is stored on disk.
    #[arg(
        long = "bearer-token",
        value_name = "TOKEN",
        requires = "url",
    ...
    for (name, cfg) in entries {
        match &cfg.transport {
            ...
            McpServerTransportConfig::StreamableHttp {
                ...
                let token_display = if bearer_token.is_some() {
                    "<inline>".to_string()
                } else {
                    bearer_token_env_var
                        .clone()
